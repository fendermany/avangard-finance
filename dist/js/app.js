 ...
 */
    ...
    (() => {
        ...
        let gotoblock_gotoBlock = (targetBlock, noHeader = false, speed = 500, offset = 0) => {
            const targetBlockElement = "string" === typeof targetBlock ? document.querySelector(targetBlock) : targetBlock;
        ...
        function formSubmit(validate) {
            ...
            const changeModalState = state => {
                const registrationInputs = document.querySelectorAll(".registration__mobile input");
            ...
            async function formSubmitAction(state, form, e) {
                ...
                if (0 === error) {
                    ...
                    if (ajax) {
                        ...
                        if (form.getAttribute("data-registration-last")) for (let key in state) formData.append(key, state[key]);
        ...
        class SelectConstructor {
            ...
            getSelectTitleValue(selectItem, originalSelect) {
                ...
                if (originalSelect.hasAttribute("data-search")) return `<div class="${this.selectClasses.classSelectTitle} _icon-arrow-down"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}"><input autocomplete="off" type="text" placeholder="${selectTitleValue}" data-placeholder="${selectTitleValue}" class="${this.selectClasses.classSelectInput}"></span></div>`; else {
                    const customClass = this.getSelectedOptionsData(originalSelect).elements.length && this.getSelectedOptionsData(originalSelect).elements[0].dataset.class ? ` ${this.getSelectedOptionsData(originalSelect).elements[0].dataset.class}` : "";
                    return `<button type="button" class="${this.selectClasses.classSelectTitle} _icon-arrow-down"><span${pseudoAttribute} class="${this.selectClasses.classSelectValue}${pseudoAttributeClass}"><span class="${this.selectClasses.classSelectContent}${customClass}">${selectTitleValue}</span></span></button>`;
                }
            }
        ...
        function rangeInit() {
            const stepsSlider = document.getElementById("steps-slider"), input0 = document.getElementById("range_min"), input1 = document.getElementById("range_max"), inputs = [ input0, input1 ];
            if (stepsSlider) {
                ...
                const setButton500 = document.getElementById("set-sliders-500"), setButton10 = document.getElementById("set-sliders-10");
                setButton500.addEventListener("click", (function(e) {
                    stepsSlider.noUiSlider.set([ 2e5, 5e5 ]);
                    e.target.classList.add("_active");
                    setButton10.classList.remove("_active");
                }));
                setButton10.addEventListener("click", (function(e) {
                    stepsSlider.noUiSlider.set([ 2e5, 1e7 ]);
                    e.target.classList.add("_active");
        ...
        function initSliders() {
            ...
            if (document.querySelector(".topbar-cars")) new core(".topbar-cars", {
            ...
            if (document.querySelector(".enter-mobile__slider")) new core(".enter-mobile__slider", {
            ...
            if (document.querySelector(".enter-desktop__slider")) new core(".enter-desktop__slider", {
        ...
        da.init();
        const script_element = document.body, switcher = document.querySelector(".switch input");
        function darkMode() {
            if ("dark" != localStorage.getItem("darkMode")) {
                script_element.classList.add("dark-mode");
                localStorage.setItem("darkMode", "dark");
            } else {
                script_element.classList.remove("dark-mode");
        ...
        if (switcher) {
            ...
            if ("dark" == localStorage.getItem("darkMode")) {
                ...
                script_element.classList.add("dark-mode");
            } else switcher.removeAttribute("checked");
        }
        let toggleMenuButton = document.querySelector(".toggleButton"), toggleMenuCloseButton = document.querySelector(".toggleMenu__close"), toggleMenuBody = document.querySelector(".toggleMenu"), header = document.querySelector(".header"), headerHeight = window.getComputedStyle(header).height;
        window.addEventListener("resize", (() => {
            headerHeight = window.getComputedStyle(header).height;
        }));
        if (toggleMenuButton) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".toggleButton")) {
                bodyLockToggle();
                document.documentElement.classList.toggle("toggle-open");
                toggleMenuBody.style.top = +headerHeight.slice(0, headerHeight.length - 2) + 8 + "px";
                if (!header.classList.contains("sticky")) toggleMenuBody.style.maxHeight = `calc(100vh - ${headerHeight})`; else toggleMenuBody.style.top = toggleMenuBody.style.maxHeight = `calc(100vh - ${headerHeight})`;
            }
        }));
        ...
        if (toggleMenuCloseButton) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".toggleMenu__close")) toggleMenuClose();
        }));
        if (toggleMenuBody) document.addEventListener("click", (e => {
            let its_toggleMenu = e.target == toggleMenuBody || toggleMenuBody.contains(e.target);
            let its_toggleMenuButton = e.target == toggleMenuButton || toggleMenuButton.contains(e.target);
            let toggleMenu_is_active = document.documentElement.classList.contains("toggle-open");
            if (!its_toggleMenu && !its_toggleMenuButton && toggleMenu_is_active) toggleMenuClose();
        }));
        const hamburger = document.querySelector(".icon-menu"), menuBody = document.querySelector(".topmenu");
        document.addEventListener("click", (e => {
            ...
            if (!its_menu && !its_hamburger && menu_is_active) menuClose();
        }));
        ...
        showMoreButtons.forEach((item => {
            ...
            item.addEventListener("click", (e => {
                ...
                if (e.target.classList.contains("_active")) item.innerHTML = "Скрыть"; else e.target.innerHTML = oldText;
            }));
        }));
        const filterButton = document.querySelector(".showfilter"), filterBody = document.querySelector(".page__filter"), filterInner = (document.querySelector(".filter__wrapper"), 
        document.querySelector(".filter__inner")), headerTop = (document.querySelector(".filter__back"), 
        document.querySelector(".filter__bigtitle"), document.querySelector(".header__top"));
        window.getComputedStyle(headerTop).height;
        if (filterBody) {
            document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".showfilter")) {
                    ...
                    if (document.documentElement.scrollTop > 0) {
                        ...
                        filterBody.style.top = +headerHeight.slice(0, headerHeight.length - 2) + 8 + "px";
            ...
            document.addEventListener("click", (function(e) {
                if (bodyLockStatus && e.target.closest(".filter__back")) hideFilter();
            }));
            document.addEventListener("click", (e => {
                let its_filter = e.target == filterBody || filterBody.contains(e.target);
                let its_filterButton = e.target == filterButton || filterButton.contains(e.target);
                let filter_is_active = document.documentElement.classList.contains("filter-open");
                if (!its_filter && !its_filterButton && filter_is_active) hideFilter();
            }));
        }
        document.addEventListener("click", (function(e) {
            if (document.documentElement.classList.contains("lock") && !e.target.closest(".showfilter") && !e.target.closest(".filter") && !e.target.closest(".icon-menu") && !e.target.closest(".topmenu") && !e.target.closest(".toggleButton") && !e.target.closest(".toggleMenu") && !e.target.closest(".popup__content")) bodyUnlock();
        }));
        if (filterBody) {
            const filterResultsButton = document.querySelector(".filter__btn"), filterResultsButtonAbsolute = document.querySelector(".filter__btn-absolute"), filterResultsButtonHeight = window.getComputedStyle(filterResultsButton).height;
            filterInner.addEventListener("scroll", (() => {
                if (filterResultsButton.getBoundingClientRect().top > window.innerHeight - +filterResultsButtonHeight.slice(0, filterResultsButtonHeight.length - 2)) {
        ...
        const auth = document.querySelector(".auth"), authListBody = (document.querySelector(".auth__online"), 
        document.querySelector(".auth__list"));
        document.addEventListener("click", (e => {
            if (e.target.closest(".auth__online")) document.documentElement.classList.toggle("auth-open");
        }));
        document.addEventListener("mouseout", (e => {
            if (e.target.closest(".auth__list")) document.documentElement.classList.remove("auth-open");
        }));
        document.addEventListener("click", (e => {
            let its_authMenu = e.target == authListBody || authListBody.contains(e.target);
            let its_authMenuButton = e.target == auth || auth.contains(e.target);
            let authMenu_is_active = document.documentElement.classList.contains("auth-open");
            if (!its_authMenu && !its_authMenuButton && authMenu_is_active) document.documentElement.classList.remove("auth-open");
        }));
        const popupRegistrationButtonNext = document.querySelector("[data-registration-next]"), popupRegistrationScreenFirst = document.querySelector(".registration__mobile-1"), popupRegistrationScreenSecond = document.querySelector(".registration__mobile-2"), popupRegistrationPaginationSpans = document.querySelectorAll(".registration__mobile-pagination span");
        ...
        const topbarList = document.querySelector(".topbar__list"), menubarList = document.querySelector(".menubar__menu-list");
        if (topbarList) dragScroll(topbarList);
        if (menubarList) dragScroll(menubarList);
        /**
 ...
 */
        const StickySidebar = (() => {
            ...
            class StickySidebar {
                ...
                static extend(defaults, options) {
                    var results = {};
                    for (let key in defaults) if ("undefined" !== typeof options[key]) results[key] = options[key]; else results[key] = defaults[key];
                    return results;
                }
                ...
                static addClass(element, className) {
                    if (!StickySidebar.hasClass(element, className)) if (element.classList) element.classList.add(className); else element.className += " " + className;
                }
                static removeClass(element, className) {
                    if (StickySidebar.hasClass(element, className)) if (element.classList) element.classList.remove(className); else element.className = element.className.replace(new RegExp("(^|\\b)" + className.split(" ").join("|") + "(\\b|$)", "gi"), " ");
                }
        ...
        const sticky_sidebar = StickySidebar;
        ...
        document.addEventListener("DOMContentLoaded", (() => {
            if (window.innerWidth >= 992) if (document.querySelector(".page__filter")) {
                new sticky_sidebar(".filter", {
